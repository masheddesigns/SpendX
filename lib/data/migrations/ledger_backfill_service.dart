...
/// Phase 1B — Idempotent financial ledger backfill & reconciliation.
///
/// This is a DATA-MIGRATION component ONLY. It is NOT a normal runtime
/// mutation path. Its single job: make the canonical `ledger_transactions`
/// journal plus an explicit `opening_balance` entry reconcile exactly to the
/// legacy stored balances, WITHOUT touching UI, providers, or stored columns.
///
/// Design constraints (locked):
///   * `ledger_transactions` = canonical money journal.
///   * `bank_accounts.balance` = legacy stored balance, used ONLY as the
///     expected value for parity (never modified).
///   * Opening balances are represented as a dedicated `LedgerType.openingBalance`
///     entry so they do not contaminate income analytics.
///   * The migration runs inside one SQL transaction; any hard failure
///     (integrity mismatch, orphan/duplicate, credit/loan parity mismatch,
///     unbackfillable transaction) throws and rolls back the ENTIRE migration.
///   * Idempotent: running it twice creates zero duplicate movements.
class LedgerBackfillService {
  // 1 cent tolerance — guards against float noise only, never hides a real
  ...
  /// Runs the full backfill inside a single transaction.
  /// On any required mismatch it throws [BackfillFailedException] carrying the
  /// report; the surrounding transaction rolls back all writes.
  ///
  /// Accepts a [Database] (wraps the steps in a transaction) or a
  /// [DatabaseExecutor] that is already inside a transaction (runs the steps
  /// inline). The latter is what the Phase 1C dry-run harness relies on so it
  /// can roll the whole run back without persisting anything.
  static Future<BackfillReport> run(DatabaseExecutor db) async {
    final report = BackfillReport();
    try {
      if (db is Database) {
        await db.transaction((txn) async {
          await _runSteps(txn, report);
        });
      } else {
        await _runSteps(db, report);
      }
      return report;
    } on _HardFail {
      report.passed = false;
  ...
  static Future<void> _runSteps(
    DatabaseExecutor txn,
    BackfillReport report,
  ) async {
    await _stepAccounts(txn, report);
    await _stepLending(txn, report);
    await _stepCredit(txn, report);
    await _stepLoans(txn, report);
    await _stepOrphanAndDuplicates(txn, report);
    await _stepAccountParity(txn, report);
    if (report.hardFailures.isNotEmpty) {
      throw _HardFail(report.hardFailures);
    }
    report.passed = true;
  }
  ...
  static Future<void> _stepAccounts(
    ...
    // 1. Per-account existing non-opening movement (baseline).
    ...
    // 2. Global transaction pass: backfill / verify every leg exactly once.
    //    (A transfer touches two accounts, so it must not be processed once
    //    per account or the second account would see the first leg's
    //    reference_id and skip its own.)
    ...
    for (final tx in transactions) {
      ...
      if (type != 'income' &&
          type != 'expense' &&
          type != 'transfer') {
        // Unknown transaction type. If it carries an account reference it
        // would change a bank balance but we cannot classify it safely.
        final acc = tx['account_id'] as String?;
        final rel = tx['related_entity_id'] as String?;
        if ((acc != null && acc.isNotEmpty) ||
            (rel != null && rel.isNotEmpty)) {
          report.hardFailures.add(
            'Unbackfillable transaction type "$type" with account '
            'reference (id=${tx['id']}). Cannot safely reconstruct.',
          );
        } else {
          report.exceptions.add(
            'Transaction type "$type" (id=${tx['id']}) has no account; '
            'deferred to Phase 2.',
          );
        }
        continue;
      ...
      if (existing.isEmpty) {
        ...
        _verifyLegs(existing, legs, tx, report);
        report.existingLedgerSkipped++;
      }
    }

    // 3. Opening balance per account = stored - (existing movement + inserted).
    for (final a in accounts) {
      final accId = a['id'] as String;
  ...
  static Future<void> _stepLending(
    ...
    for (final l in lendings) {
      ...
      final original =
          (l['original_amount'] as num?)?.toDouble() ?? 0.0;
      ...
      if (exist.isEmpty) {
        final legType =
            type == 'borrowed' ? 'lending_received' : 'lending_given';
        await db.insert('ledger_transactions', {
      ...
      report.lendingParity.add(
        BackfillParity(id, expected, derived, diff),
      );
      if (diff > tolerance) {
        report.hardFailures.add(
  ...
  static Future<void> _stepLoans(
    ...
    for (final loan in loans) {
      ...
      final total = (loan['total'] as num?)?.toDouble() ??
      ...
      for (final inst in loanInst) {
        if ((inst['status'] as String?) == 'paid') {
          ...
          if (exist.isEmpty) {
            final principal = (inst['principalComponent'] as num?)
                    ?.toDouble() ??
                0.0;
            await db.insert('ledger_transactions', {
  ...
  // ---------------------------------------------------------------------------
  // STEP 11: orphan / duplicate detection
  // ---------------------------------------------------------------------------
  static Future<void> _stepOrphanAndDuplicates(
    ...
    for (final row in allLedger) {
      ...
      if (acc != null && !accountIds.contains(acc)) {
        report.orphanCount++;
        report.hardFailures
            .add('Orphan ledger row references missing account $acc');
      }
      if (card != null && !cardIds.contains(card)) {
        report.orphanCount++;
        report.hardFailures
            .add('Orphan ledger row references missing card $card');
      }
      if (loan != null && !loanIds.contains(loan)) {
        report.orphanCount++;
        report.hardFailures
            .add('Orphan ledger row references missing loan $loan');
      }
      if (ref is String &&
    ...
    byRefType.forEach((key, count) {
      if (count > 1) {
        ...
        report.hardFailures
            .add('Duplicate ledger representation: $key x$count');
      }
    });
  }
  ...
  static Future<void> _stepAccountParity(
    ...
    for (final a in accounts) {
      ...
      report.accountParity.add(
        BackfillParity(accId, stored, derivedVal, diff),
      );
      if (diff > tolerance) {
        report.hardFailures.add(
  ...
  static String _accountMovementSql({required bool excludeOpening}) {
    final positive = excludeOpening
        ...
        : "('income', 'lending_received', 'loan_disbursement', 'refund', 'opening_balance')";
    return '''
      SELECT SUM(CASE
  ...
  static List<Map<String, dynamic>> _expectedLegs(
  ...
  static void _verifyLegs(
    ...
    for (final leg in legs) {
      final match = existing.any((r) {
        ...
        final rAmt = r['amount']?.toDouble() ?? 0.0;
        final lAmt = leg['amount']?.toDouble() ?? 0.0;
      ...
      if (!match) {
        report.hardFailures.add(
          'Ledger integrity mismatch for transaction ${tx['id']}: expected '
          '${leg['type']} on ${leg['account_id']} amount ${leg['amount']} '
          'not present in existing ledger rows.',
        );
      }
    }
  }
...
class BackfillReport {
  ...
  String get verdict {
    if (passed) {
      return exceptions.isEmpty
          ? 'PASS'
          : 'PASS WITH DOCUMENTED MIGRATION EXCEPTIONS';
    }
    return 'FAIL — ROLLED BACK';
  }
  ...
  Map<String, dynamic> toSummary() => {
        ...
        'accountParityPassed':
            accountParity.where((p) => p.passed).length,
